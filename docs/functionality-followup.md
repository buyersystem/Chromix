...
该发布修复在 [34823050178](https://github.com/xiaozhou26/Chromix/actions/runs/34823050178)
通过 Python/Node 两个 job；同次契约 CI 两个平台遇到上游 HTTP 503。源码获取
因此增加最多四次、2/4/8 秒退避的临时传输错误重试，每次连接/读取仍设超时。
404、证书错误、过大响应、非法 Base64 和 SHA-256 不符立即失败，不重试内容
校验错误，不切换来源或使用补丁派生 preimage。重试与发布检查共 34 项回归通过。
